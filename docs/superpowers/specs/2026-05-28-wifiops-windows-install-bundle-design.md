...
- Allow the install directory to be overridden with `WIFIOPS_INSTALL_DIR`.
...
3. Create `%LOCALAPPDATA%\WifiOps\.venv` by default, or `%WIFIOPS_INSTALL_DIR%\.venv` when overridden.
4. Install the bundled WifiOps wheel with `--no-index --find-links`, without upgrading pip or downloading from the internet.
5. Copy launchers into the install directory's `bin` folder.
...
```cmd
WIFIOPS_BIN_DIR=%~dp0
WIFIOPS_APP_DIR=<parent of WIFIOPS_BIN_DIR>
WIFIOPS_CONFIG=%WIFIOPS_APP_DIR%\config.yaml
```

and then call the installed venv entrypoint. The app directory must be resolved
relative to the launcher location so custom install directories work after
install.
...
- Installer docs and tests cover the offline wheelhouse contract and custom install directory behavior.
